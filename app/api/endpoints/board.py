...
# 게시판 읽기 - 2
@api_router.get("/{board_id}", status_code=200, response_model=BoardInDBBase)
def fetch_board(
    board_id: int,
    db: Session = Depends(deps.get_db),
) -> dict:
...
# 게시판 읽기 3
@api_router.get("/user/{user_id}", status_code=200, response_model=List[BoardInDBBase])
def get_user_posts(
    user_id: int,
    db: Session = Depends(deps.get_db),
) -> dict:
...
# 게시판 쓰기
@api_router.post("/", status_code=201, response_model=Board)
def create_board(
...
# 게시판 수정
@api_router.patch("/{board_id}", status_code=200, response_model=Board)
def update_board(
    board_id: int,
...
# 게시판 삭제
@api_router.delete("/{board_id}", status_code=200, response_model=Board)
def delete_board(
    board_id: int,
    db: Session = Depends(deps.get_db),
) -> dict:
